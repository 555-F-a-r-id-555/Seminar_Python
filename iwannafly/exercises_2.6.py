...
for i in range(len(matrix)):
    for j in range(len(matrix[0])):
        print(matrix[(i - 1) % n][j] + matrix[(i + 1) % n][j] + matrix[i][(j - 1) % m] + matrix[i][(j + 1) % m],
              end=' ')
    print()
...
def simulate_input(_input=stdin):
    matrix = []
    while True:
        row = _input.readline().split()
        if row == ['end']:
            break
        matrix.append([int(elem) for elem in row])
...
assert simulate_input(StringIO(input_data)) == expected_output